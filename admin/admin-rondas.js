...
let _marcadoresMapaLog = [];
let _linhaDistanciaMapa = null;
...
function abrirMapaLogRonda(pLat, pLng, lLat, lLng, nomePonto, nomeFunc) {
  ...
  // Markers
  const m1 = L.marker([pLat, pLng], {icon: iconEscola}).addTo(_mapaLogAuditoria).bindPopup('<b>QR Code Físico</b><br>' + nomePonto).openPopup();
  const m2 = L.marker([lLat, lLng], {icon: iconVigia}).addTo(_mapaLogAuditoria).bindPopup('<b>Local da Batida</b><br>' + nomeFunc);
  
  // Linha
  _linhaDistanciaMapa = L.polyline([[pLat, pLng], [lLat, lLng]], {color: 'red', dashArray: '5, 5'}).addTo(_mapaLogAuditoria);

  // Ajustar zoom para caber os dois
