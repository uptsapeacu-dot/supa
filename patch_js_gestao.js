...
// 2. Arquivar lotações ao desligar
const salvarFuncOriginal = `if (novoStatus !== 'ativo') {
          await clienteSupabase.from('historico_status_funcionarios').insert([{`;
const salvarFuncNovo = `if (novoStatus !== 'ativo') {
          await clienteSupabase.from('historico_status_funcionarios').insert([{`;
// Wait, a better injection point for the archive logic is right after updating the status of the employee,
// inside salvarFuncionario.
if (!js.includes('update({ ativo: false }).eq(\'funcionario_id\', funcionarioId)')) {
    // Look for:
    ...
    // It's easier to put the logic right before or after the 'vinculos_funcionarios' is inserted in salvarFuncionario
    // Actually, wait! The user said: "arquivar lotacao na verdade vai acontecer quando o status do funcionario que possue aquele lotacao mudar para desligao"
    // At the end of salvarFuncionario:
    const injetarArquivamento = `
    ...
    js = js.replace(/fecharModalFuncionario\(\)/g, injetarArquivamento.trim() + '\n    fecharModalFuncionario()');
}
